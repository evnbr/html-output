...
function make_tab(filename, content) {
  ...
  // Codemirror extended by: http://stackoverflow.com/questions/16274039/codemirror-make-atomic-range-of-token
  editor.on("tokenParsed", function(cm, start, end, style, text) {
      if(!cm.findMarksAt(end).length) { //check if the mark doesn't exist yet
          if(style && style.contains("number")) {
              // cm.markText(start, end, {
              //   atomic: true,
              //   clearOnEnter: true,
              //   replacedWith: getNumberSlider(text, start, end)
              // });
          }
          else if(style && style.contains("color")) {
              cm.setBookmark( start, {
                // widget: getColorPicker(text, start, end),
                insertLeft: true
              });
              ...
              cm.markText(start, end, {
                atomic: true,
                clearOnEnter: true,
                replacedWith: getColorPicker(text, start, end)
              });
          }
      }
  });
